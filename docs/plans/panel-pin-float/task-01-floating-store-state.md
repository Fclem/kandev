...
- **Transactions with journal + unified persistence guard:** float/dock/restore capture+validate → mutate → commit (store + blob + `persistEnvLayoutNow`); on a mid-mutation throw, re-apply the journaled LayoutState, drop the partial entry, and keep the previous blob. A store-owned transaction token (`floatingTransactionToken`, token-guarded begin/end actions read by `setupLayoutPersistence` — no module-local flag, no import cycle) gates every layout-persistence entry point (`persistNow`, the debounce callback, the `onDidLayoutChange` handler, `beforeunload`) through one `canPersistLayout()` guard; an already-scheduled timer is cancelled/held and marked dirty on transaction begin; the token resets on unmount. **Storage commits fail closed:** a failed blob write rolls the transaction back (journal re-apply) and the group stays pinned with a console warning — never a lost panel. Suppression cleanup runs in `finally`, token-guarded.
