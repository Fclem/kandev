...
- **Layer inventory (mandatory, auditable deliverable):** the task ships a
  **callsite table** (tracked file + completion report) listing every
  ...
  **host/plugin boundary** is defined: plugin-DOM layers register through a
  host API (`host.ui.registerFloatingOwnedLayer`-style exposed to task
  panels), since the plugin registry is reactive only for registrations, not
  an owned-layer lifecycle boundary. One real test per primitive family,
