...
- **Layer inventory (mandatory, auditable deliverable):** the task ships
  `docs/plans/panel-pin-float/owned-layer-inventory.md` — a callsite table
  listing every floating-capable panel's interactive layer owners (Radix
  Dialog/Popover/DropdownMenu/ContextMenu/HoverCard inside chat, plan,
  terminal, files, changes, diff, plugins, editors), each row naming the exact
  component/file, the primitive family, and the registration mechanism. The
  **host/plugin API is concrete**: `host.ui.registerFloatingOwnedLayer(layerRoot: HTMLElement) => () => void` added to the plugin SDK/types and implemented in `lib/plugins/host-api.ts` (idempotent; unregister on close, unmount, and plugin unregistration; plugin ownership validated against the registered task panel; registry cleanup wired into `unregisterPlugin`). One real test per primitive family, including a plugin-panel layer.
