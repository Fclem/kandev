...
import { DEFAULT_LOCALE, i18n } from "./index";

/**
 * Locale-aware formatting helpers. The active locale comes from the shared
 * Lingui `i18n` instance. `pseudo` is a QA locale with no real CLDR data, so
 ...
 * The bucket strings are routed through Lingui so they extract and pseudolocalize.
 */
export function formatRelative(dateStr: string, now: number = Date.now()): string {
  ...
  if (diffHr < 24) return t`${diffHr}h ago`;
  const diffDay = Math.floor(diffHr / 24);
  return t`${diffDay}d ago`;
}
