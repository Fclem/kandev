...
- **Migration + role bootstrap ordering:** `migrateEnvLayoutV3(raw, envId)` runs once (versioned marker) on the raw v3 native JSON BEFORE `fromJSON`, assigning `logicalId` UUIDs and `role` (right = pinned column containing files/changes; tie → leftmost; none → no pinned-right role), persisting v4 only after a validated apply; later restores consume only stored roles (never re-inferred). A synchronous role bootstrap normalizes live `LayoutColumn.role` and rebuilds the in-memory `rootColumns` sidecar BEFORE any `hasLivePinnedRightColumn` call (first mount with empty/absent blob included), then derives visibility/enforcement, then schedules complete-pair persistence; first-mount, empty-blob, preset/reset-invalidation, and immediate-toggle-before-debounce tests cover the ordering.
