...
- A new optional panel, **Prompt history**, is available from the task workbench "+" menu (the `AddPanelMenuItems` dropdown) on the desktop workbench and the Office workbench, next to the existing Todos entry. It is not part of the phone bottom navigation.
...
- Phone entry point (product decision, recorded here so it is intentional rather than an accident of the desktop workbench): the phone surface is a transcript-first, one-dimensional composition (bottom nav: chat/plan/changes/files/terminal/review plus a plugin picker) with no workbench "+" menu. Prompt history is a dense at-a-glance review list; its value on phones is served by the transcript itself, which is one tap away and already offers per-prompt jump arrows on user messages and per-turn run durations. Adding a seventh dense panel to the phone bottom nav would compete with that focused flow. Desktop keeps the panel; phones keep the transcript path. Mobile parity is satisfied by the existing mobile transcript fallback coverage: `apps/web/e2e/tests/chat/mobile-last-prompt-scroll.spec.ts` proves the phone's scroll-to-last-prompt and scroll-to-start controls (the last/start transcript handles this feature's arrow builds on). It does not exercise per-user-message navigation arrows — the panel itself is desktop-only, so the arbitrary-message arrow path has no mobile surface, and this shared last/start-handle coverage is the extent of the mobile parity claim. Only the desktop panel is new UI; no new `mobile-*.spec.ts` is required because no mobile surface changes.
...
- **GIVEN** a prompt row in the panel, **WHEN** the user clicks its arrow button, **THEN** the session chat panel becomes active and the transcript scrolls so that prompt message sits at the top of the transcript viewport.
