...
function DependenciesSection({ task }: { task: Task }) {
  ...
  return (
    <>
      <PropertyRow label={t("task:blockedBy")} valueClassName="ml-auto" alignStart>
...
function TimelineSection({ task }: { task: Task }) {
  ...
  return (
    <>
      ...
      <PropertyRow label={t("task:completed2")}>{dateOrDash(task.completedAt)}</PropertyRow>
      <PropertyRow label={t("task:created2")}>{formatDate(task.createdAt)}</PropertyRow>
