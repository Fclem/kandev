...
- **Transactions: one coordinator + journal + phase model:** a single coordinator (`floating-transaction.ts`) owns the per-env operation/generation and phase state (`begin → advance → settle`); while busy, float/dock/reset/toggle-right actions are **rejected** (the pin control renders disabled) — no re-entrant transactions; re-entrancy tests cover mutation, portals-adopted, rollback, and stale-cleanup phases. A per-env **operation journal** (`kandev.dockview.env-floating-journal.<envId>`) is written **before** mutation with complete before/after blob+layout snapshots; **`recoverFloatingJournalOnce(api, envId)`** is the single idempotent pre-restore gate (generation-tagged, runs before every restore entry) and recovers deterministically (after-state if the mutation completed and both keys validate, else before-state; journal cleared after). **Size budget preflight** (96 KB default per env: blob + journal) fails non-destructively before any mutation. Recovery runs the phase model `mutating → restoring → portals-adopted → persist-recovered → settled`; only the portals-adopted phase persists the journaled layout through **status-returning APIs** (the current `setEnvLayout`/`persistEnvLayoutNow` swallow errors — new APIs are part of the contract); token cleanup is generation-guarded at settled. Journal-free restore is idempotent with **per-panel salvage** (conflicting panels follow the live identity; non-conflicting tabs are retained and re-docked per the collision policy — nothing disappears silently). A store-owned transaction token gates every persistence entry point (`persistNow`, debounce callback, `onDidLayoutChange`, `beforeunload`) through one `canPersistLayout()`; the unload path is **one idempotent transaction-aware handler** (existing listener replaced, never duplicated) writing the journaled pre-transaction layout; an already-scheduled timer is cancelled/held on begin. **Single storage policy: fail closed** on every blob-write failure (private mode included) — rollback to pinned; no ephemeral floating mode.
