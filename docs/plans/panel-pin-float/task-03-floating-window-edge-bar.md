...
- **Owned-region coordinator** (`dockview-floating-coordinator.ts`, pattern: `hooks/use-panel-search.ts`): a floating window's region = its DOM subtree plus any Radix layer opened from within it (`useFloatingOwnedLayer` hook wired to the existing `onOpenChange` handlers; layers stay owned until closed). Collapse triggers: window-capture pointerdown whose target is in no owned region — **deferred while an owned layer is open** (pending collapse applied when the last owned layer of that window closes via its dismiss callback; cancelled if the pointerdown landed inside an owned region); `focusout` whose `relatedTarget` (checked after a microtask, for Radix portal focus moves) is in no owned region; or Escape per the Escape contract. Only the focused/last-interacted expanded window collapses; other floating groups keep their display state.
...
- Risk: pointerdown deferral must cancel correctly (outside click while a menu is open collapses only after dismissal; a click inside the owned layer cancels the pending collapse); tested with real Radix pointer ordering, not synthetic events.
