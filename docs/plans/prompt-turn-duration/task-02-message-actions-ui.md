...
- **Inputs:**
  ...
  - Mobile parity (required, not optional): the duration is one more inline-flex span in the existing action row — it must not change the row's composition in a way that clips or wraps on narrow widths, must not add a scroll container, and must not alter touch behavior (below `sm` the row is always visible; at `sm+` it reveals with the row via `group-hover`/`focus-within`). Run `/mobile-parity` before finishing and record the rendered mobile verification in `## Results`; the mobile E2E scenario lives in task-03.
