...
the existing env layout / maximize persistence. **Revision 45 incorporates the
round-44 adversarial review** (this package has been adversarially reviewed
...
8. **Identity coordination (session + right panels).** Session replacement is
   ...
   `shouldSkipPanelEnsure` (skips EVERY floating-owned session id via the
   atomic `floatingSessionIds` query — winner AND non-winner — while
   `floatingSessionWinner` compare-and-clear remains for replacement
   ownership; unrelated grid-only siblings
...
- **Unload phase table:** mutating/unverified ⇒ BEFORE + aborted journal;
  only verified-AFTER ⇒ AFTER + `mutating` journal, verified read-back, then
   clear (NO `committed` value exists anywhere — the verified pair is the
   sole terminal evidence); failures retain the journal.
