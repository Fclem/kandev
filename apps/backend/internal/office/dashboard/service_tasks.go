...
// parent (via the canonical detacher). A non-empty value also applies the
// canonical re-parent workspace policy: an inherit_parent subtask keeps its
// materialized workspace as shared_group instead of inheriting the new
// parent's. Rejects only direct self-reference (parentID == taskID); deeper
// cycle detection is intentionally not enforced on this surface.
