...
- Clicking it calls `floatGroup(groupLogicalId)` / `dockGroup(groupLogicalId)` — the header resolves the native `group.id` via `resolveLogicalGroupId(nativeGroupId, normalizedLiveRegistry)` and REJECTS unmapped native ids (stale native ids after regeneration never silently target the wrong group); when the group is floating (no grid header), the control is instead rendered by the floating window / collapsed bar (task-03) and reports the unpinned state.
