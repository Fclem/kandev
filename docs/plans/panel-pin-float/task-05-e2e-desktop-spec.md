...
- Mobile: the dockview workbench does not render on phone viewports — the mobile task surface (`mobile-task-layout` + `session-mobile-bottom-nav` / the mobile right-panel surface) keeps panels reachable, and floating/collapse is intentionally absent from the mobile state model. No existing mobile spec proves panel reachability, so this task adds one narrow `mobile-*.spec.ts` retained-path scenario naming the exact mobile entry control (the mobile task page's panel affordance), asserting Plan/Changes/Files/Terminal remain reachable on a phone viewport, and including viewport-containment/no-horizontal-overflow checks where relevant; plus the written justification.
