...
- The browse endpoint validates no lookback token, so the two paths differ for a value outside the accepted set: a project-scoped browse withholds the parameter and answers 200 with no age constraint, while an organization-scoped browse forwards it, so a value the provider can also parse (`30m`) is applied as that endpoint's time range and a value it cannot (`nonsense`) is rejected. The dialog only sends an offered token, so both paths are API-only, and failing the project-scoped browse closed as well would add a second validation site for a path that dispatches nothing.
