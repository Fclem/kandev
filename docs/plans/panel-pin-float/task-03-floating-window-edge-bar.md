...
- **Layer inventory (mandatory, auditable deliverable):** the task ships an inventory table (in the task's completion report) listing every floating-capable panel's interactive layer owners — Radix Dialog/Popover/DropdownMenu/ContextMenu/HoverCard inside chat, plan, terminal, files, changes, diff, plugins, editors — mapped to their `useFloatingOwnedLayer` registration; the host/plugin boundary for registering plugin-DOM layers is defined (the plugin registry is reactive for registrations, not an owned-layer lifecycle boundary). One real test per primitive family, including a plugin-panel layer.
