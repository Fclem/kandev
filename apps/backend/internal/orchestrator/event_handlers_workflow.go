...
// dispatchTakenQueuedMessage keeps the legacy session-ID-only dispatch seam
// available for handoff-aware drains. Resolve the current identity before
// delegating so those drains receive the same replacement-session protection
// as the newer identity-aware queue paths.
