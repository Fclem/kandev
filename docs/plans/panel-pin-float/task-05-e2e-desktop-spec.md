...
- New Playwright spec `apps/web/e2e/tests/task/panel-pin.spec.ts` proves on a real task workbench:
  - unpin floats the group over the view with the freed space reflowed; outside click collapses to a **vertical** edge bar for a right-column group and a **horizontal** bottom bar for a bottom/center group;
  ...
  - **keyboard collapse**: tabbing out of the expanded window collapses it without a pointer event; Escape collapses only when focus is inside the window;
...
- Spec: Scenarios (all), Persistence guarantees, Integration contract, Out of scope (mobile).
...
- Risk: terminal-liveness and reload-recreation assertions depend on backend state transitions — use `watchWs`/`waitForHttp` causal helpers and `expect.poll` against the API client, never hand-picked timeouts.
