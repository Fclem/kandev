...
- **Transactions with journal + crash-consistent commit:** float/dock/restore capture+validate → mutate → commit → on a mid-mutation throw, re-apply the journaled LayoutState **as a portal-safe restore transaction** (restore gate around `fromJSON`, target-env exclusion sets, gate cleared only after portal adoption — rollback never releases a portal or parks/stops a terminal/vscode process). Commit writes the floating blob first, then the env layout, both with **write status** (the current `setEnvLayout`/`persistEnvLayoutNow` swallow errors — new status-returning APIs are part of the contract); a layout-write failure after a successful blob write re-writes the previous blob (best effort) and rolls back to pinned. `beforeunload` during a transaction writes the journaled pre-transaction layout, never `api.toJSON()`. A store-owned transaction token (`floatingTransactionToken`, token-guarded begin/end actions read by `setupLayoutPersistence` — no module-local flag, no import cycle) gates every layout-persistence entry point (`persistNow`, the debounce callback, the `onDidLayoutChange` handler, `beforeunload`) through one `canPersistLayout()` guard; an already-scheduled timer is cancelled/held and marked dirty on transaction begin; the token resets on unmount. **Single storage policy: fail closed** on every blob-write failure (private mode included) — rollback to pinned with a console warning; there is no ephemeral floating mode.
- **Persistence:** versioned `EnvFloatingState` blob under `kandev.dockview.env-floating.<envId>` with a type guard (mirroring `isEnvMaximizeState`) that validates each panel definition independently (JSON-safe values, params ≤ 64 KB serialized, depth-bounded recursive validation), enforces finite nonnegative numeric bounds (`order`/`nextOrder` ≤ 10 000, `size` ≤ 100 000, `columnIndex` ≤ 64), **normalizes `nextOrder` on load to max(accepted group orders) + 1** (a stale counter cannot break monotonicity; exceeding the cap makes allocation fail non-destructively), rejects duplicate panel/group ids deterministically (first wins), defaults to `{}`; `nextOrder` monotonic counter with a documented **exhaustion policy** (at the cap, a new float fails non-destructively and the group stays pinned — no clamping or reuse); saved on env switch (`saveOutgoingEnv`), restored after layout restore, cleared by reset and `cleanupTaskStorage`.
