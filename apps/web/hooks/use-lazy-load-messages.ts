...
function loadTargetsReached(args: {
  ...
  const { loadedTextParts, minTextPartsPerLoad } = args;
  return !minTextPartsPerLoad || loadedTextParts >= minTextPartsPerLoad;
}
...
async function loadPagesToTargets(args: {
  ...
  const { fetchPage, minTextPartsPerLoad } = args;
  let total = 0;
  ...
  for (let page = 0; page < MAX_PAGES_PER_LOAD; page++) {
    ...
    if (loadTargetsReached({ loadedTextParts, minTextPartsPerLoad })) break;
  }
  return total;
}
...
function useOlderPageFetcher(sessionId: string | null, stateRef: { current: LazyLoadState }) {
  ...
  const fetchPage = useCallback(
    async (mode: "visible" | "raw" = "visible") => {
      ...
      // concurrent caller (automatic backfill or a last-prompt preload during
      // transcript loading) joins the existing promise (first-request-wins)
      // instead of skipping.
...
export function useLazyLoadMessages(sessionId: string | null, options?: LazyLoadMessagesOptions) {
  const { minTextPartsPerLoad } = options ?? {};
  ...
  // An optional per-consumer target accumulates matching rows across pages
  // until the threshold, exhaustion, a zero-result page, or the safety cap.
  // Raw rows still determine progress and cursor movement.
  const loadMore = useCallback(async () => {
    if (!minTextPartsPerLoad || !sessionId) {
      return (await fetchPage()).count;
    }
    setIsAccumulating(true);
    try {
      return await loadPagesToTargets({ fetchPage, minTextPartsPerLoad });
    } finally {
      setIsAccumulating(false);
    }
  }, [fetchPage, minTextPartsPerLoad, sessionId]);
