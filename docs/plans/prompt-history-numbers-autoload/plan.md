...
Build order: backend message contract first (`prompt_index`, read-time derived, no migration), then the frontend numbering display, then the panel auto-load sentinel, then E2E coverage plus the test-harness `author_type` passthrough that E2E seeding needs. All four land in one PR; tasks are sequential because each frontend step consumes the previous step's contract.

## Backend

### 1. Message contract: `prompt_index` (Task 01)

The message JSON gains an optional `prompt_index` (1-based ordinal among the session's user messages, ordered by normalized microsecond `created_at` asc, ties by `id` asc — identical to the panel's entry ordering). The web store receives it from paginated list/boot responses and indexed WS replay responses; ordinary single-message/full-list consumers remain on legacy scans. Indexed reads derive ordinals over the session in a single normalized pass — the window aggregate for pagination, the shared correlated count for single-row replay/update reads — so no data-column or table-shape migration is needed; one additive expression index keeps each pass index-only with no per-page sort. Live user-message creation assigns the same normalized ordering key inside its per-session write boundary before event publication.
...
- `apps/backend/internal/task/repository/sqlite/message.go` — keep hot `GetMessage` and full `ListMessages` 12-column; add dedicated `GetMessageWithPromptIndex` (correlated count) and the computed `prompt_index`/`scanPromptIndexedMessageRows` to the paginated ordered CTE/window path and the single-row read. Keep all other legacy scanner/projection callers unchanged.
- `apps/backend/internal/db/dialect/` — provide dialect-specific normalized-microsecond expressions for the window `ORDER BY`, the outer cursor predicate/ordering, and the single-row count predicate, normalizing any SQLite `±TZ` offset to UTC (see Task 01).

The indexed query computes:

```sql
CASE WHEN author_type = 'user' THEN
  SUM(CASE WHEN author_type = 'user' THEN 1 ELSE 0 END)
    OVER (
      PARTITION BY task_session_id
      ORDER BY normalized_microsecond(created_at), id
      ROWS UNBOUNDED PRECEDING
    )
ELSE 0 END AS prompt_index
```

The expression is used inside an ordered CTE that spans the FULL session for `ListMessagesPaginated`; the cursor/`id` predicate is applied only in the outer query. `GetMessageWithPromptIndex` derives its single-row ordinal from a correlated COUNT over user rows using the same normalized-microsecond predicate as the live-create transaction, so live, replay, and read ordinals agree by construction. The outer pagination ORDER BY and cursor predicate use the same normalized key, so pages are contiguous in ordinal order and the sentinel `promptNumber === 1` stop cannot skip a higher ordinal; the cursor bound is pre-normalized to the exact `YYYY-MM-DD HH:MM:SS.ffffff` UTC string form (space separator, 6-digit fraction, no `Z`) before binding — never a serialized RFC3339Nano `time.Time` — and the CTE applies NO limit: `LIMIT (limit+1)` lives in the outer SELECT so `prompt_index` stays absolute (see Task 01). Each indexed request (an older-page fetch or a single-row replay/update read) runs one O(session) normalized pass over the session — the documented bound, made index-only (no per-page sort) by the one additive expression index `(task_session_id, normalized_microsecond(created_at), id)`; no data-column or table-shape migration is needed. `normalized_microsecond` is a dialect helper, not literal SQL. It truncates legacy and new SQLite/PostgreSQL timestamps to pinned UTC microseconds before ordering — applying any embedded SQLite `±TZ` offset so the key is true UTC — while the frontend retains full nanoseconds only for duration subtraction.
