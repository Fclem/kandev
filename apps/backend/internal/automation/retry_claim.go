...
func (s *Store) PromoteClaimedRetry(ctx context.Context, runID, token string, generation int64) error {
	result, err := s.db.ExecContext(ctx, s.db.Rebind(`UPDATE automation_runs SET retry_state = ?, status = ? WHERE id = ? AND retry_state = ? AND status = ? AND retry_claim_token = ? AND retry_group_generation = ?`), RetryStateTriggered, RunStatusTriggered, runID, RetryStateClaimed, RunStatusScheduledRetry, token, generation)
	if err != nil {
		return err
	}
