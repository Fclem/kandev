...
Browsers match cookies by host and ignore the port, so several auth-enabled kandev instances on the same host (same IP, different ports) would otherwise share one cookie jar. Kandev isolates them by port-scoping its instance-identity cookie **names**: `kandev_session_<port>`, `kandev-active-workspace_<port>`, and `office-active-workspace_<port>` on a ported host, plain names on a default-port host. The default scheme ports 80 and 443 are treated as no-port (browsers omit them from `Host`, and the SPA's API-origin derivation reports no port for them), so `example.com:80` and `example.com` resolve to the same plain names. Logging into one instance no longer logs the others out, and selecting a workspace in one no longer changes what the others boot into.
