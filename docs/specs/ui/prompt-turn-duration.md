...
# Prompt Turn Duration on Message Hover
...
- The hover action row of a **user prompt** message (the `MessageActions` row: copy, raw toggle, navigation, metadata, timestamp) additionally shows the duration of the turn that the prompt produced, when that turn has completed.
...
- The duration is the wall-clock time from the prompt's `created_at` until its turn's `completed_at`, rounded down to whole seconds and clamped at zero. The timestamp parsing, flooring, and clamping arithmetic — and the compact `formatPromptDuration` display — are the same as the Prompt history panel's (`docs/specs/ui/prompt-history-panel.md`, "Duration rules"). The end bound is the turn's completion only: the panel's additional "earlier of turn completion and next prompt" bound does NOT apply here, because this row shows the duration of the turn that resulted from this prompt and only after that turn has completed. The duration appears only when both timestamps are present and parseable.
...
- The duration is part of the existing hover-reveal row: visible on hover/focus exactly like the row's other controls, with no new interaction surface.
...
- **GIVEN** a completed-turn user prompt in the transcript on a viewport at or above the `sm` breakpoint, **WHEN** the row is not hovered/focused, **THEN** the duration is hidden with the rest of the row; **WHEN** the row is hovered or keyboard-focused, **THEN** the duration is visible.
- **GIVEN** a completed-turn user prompt in the transcript below the `sm` breakpoint (coarse-pointer layouts), **WHEN** the row renders, **THEN** the duration is visible with the rest of the row, which is always visible there (`opacity-100` base class, unchanged by this feature).
...
- Changes to the row's reveal behavior on any breakpoint: below `sm` the row remains always visible (existing `opacity-100` base class) and at `sm+` it remains hidden until hover/focus, exactly as today.
