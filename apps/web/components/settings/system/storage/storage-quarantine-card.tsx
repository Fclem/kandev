      ...
      <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0 space-y-1">
          ...
          <p className="break-all text-[11px] text-muted-foreground">
            <Trans i18nKey="settings:trash" values={{ quarantine_path: entry.quarantine_path }}>
          ...
          <p className="text-xs text-muted-foreground">
            {eligible ? (
              <span className="text-emerald-600">{t("settings:eligibleNow")}</span>
            ) : (
              <>
                {t("settings:protectedUntil")}{" "}
    ...
    <CardHeader>
      ...
      <div className="flex flex-col gap-2 sm:flex-row">
        <StorageActionButton
          ...
          disabledReason={
            bulkDisabledReason ??
            (counts.eligible === 0 ? t("settings:noQuarantineEntriesAreEligibleYet") : undefined)
          }
          onClick={() => onPurge("eligible")}
          data-testid="storage-quarantine-clear-eligible"
        >
          {t("settings:clearEligible")}
        </StorageActionButton>
        <StorageActionButton
          variant="destructive"
          className="w-full sm:w-auto"
        ...
        >
          {t("settings:forceClearAll")}
        </StorageActionButton>
      </div>
      <CardDescription>
        <Trans
          i18nKey="settings:cleanupMovesRecoverableDataHereFirst2"
          values={{ eligible: counts.eligible, protected: counts.protected }}
        >
          ...
          {counts.eligible} eligible now · {counts.protected} protected
        </Trans>
      </CardDescription>
      <p className="text-xs text-muted-foreground" data-testid="storage-quarantine-schedule-copy">
        {schedulingEnabled
          ? t("settings:eligibleEntriesRemovedByFirstMaintenanceRun", {
              count: checkIntervalHours,
            })
          : t("settings:automaticQuarantineCleanupIsOffRun")}
      </p>
    </CardHeader>
  );
}
