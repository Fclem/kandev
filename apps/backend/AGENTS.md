...
- `i18n.T(locale, key)` / `i18n.Tf(locale, key, vars)` for lookup; `Tf`
  interpolates `{{name}}`, mirroring the frontend's `t(key, vars)`.
- `i18n.FromRequest(r)` at the HTTP boundary; `i18n.ContextWithLocale` /
  `i18n.FromContext` when the rendering code is too far from the request to hold
  one. A context that never passed through HTTP yields `en`.
