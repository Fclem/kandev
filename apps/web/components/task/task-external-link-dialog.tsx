...
type TaskExternalLinkTarget = {
  id: string;
  /** The task's own title — user data, never translated. */
  title: string;
};
...
const PROVIDERS: Record<ExternalLinkProvider, ProviderConfig> = {
  jira: {
    ...
    descriptionKey: "task:useAJiraTicketKeyOr",
    inputLabelKey: "task:ticket",
    placeholderKey: "task:proj123OrPasteTicketUrl",
    validationHintKey: "task:pasteAJiraTicketUrlOr",
  ...
  linear: {
    ...
    descriptionKey: "task:useALinearIssueIdentifierOr",
    inputLabelKey: "task:issue",
    placeholderKey: "task:eng123OrPasteIssueUrl",
    validationHintKey: "task:pasteALinearIssueUrlOr",
  ...
  sentry: {
    ...
    descriptionKey: "task:useASentryShortIdOr",
    inputLabelKey: "task:issue",
    placeholderKey: "task:proj123OrPasteIssueUrl",
    validationHintKey: "task:pasteASentryIssueUrlOr",
...
function useExternalLinkForm(
  ...
  const submit = async () => {
    ...
    if (config.requiresInstance && !instanceId) {
      setError(t("task:selectASentryInstanceToLink"));
      return;
    }
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : t("task:failedToLinkItem", { item: t(config.inputLabelKey) }),
      );
    } finally {
      setSubmitting(false);
    }
