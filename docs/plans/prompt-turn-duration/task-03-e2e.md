...
- **Inputs:**
  ...
  - Row locator: the transcript's stable outer row is `#msg-<key>` (`apps/web/components/task/chat/message-list-native.tsx`, `MessageRow`) — always scoped under `session.activeChat()`, never `page`. There is NO `chat-message` test id/class to select — do not invent one. The duration is inside the `MessageActions` row of the user message; the row's reveal is `opacity`-based (base `opacity-100`, `sm:opacity-0 sm:group-hover:opacity-100 focus-within:opacity-100`), so hover assertions use computed opacity, not visibility.
