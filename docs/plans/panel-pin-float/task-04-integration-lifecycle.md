...
- **Restore call sites (exhaustive):** `recoverFloatingJournalOnce(api, envId)` runs before every restore entry, then `restoreFloatingAfterLayout` (in `lib/state/dockview-floating.ts`, idempotent) runs before `isRestoringLayout` clears at every completion point, each with one focused test:
  ...
  3. **maximize restore** (`restoreMaximizeFromStorage`) — defined ordering: journal first, floating session entries reconciled (winner written before the auto-session hook in this branch), floating state restored against the saved `preMaximizeLayout` without mutating the two-column overlay;
