...
- **Task lifecycle events:** Ordinary task mutations publish through Service/Publish helpers; durable archive-cascade mutations use the aggregate-owned revisioned outbox only. Never call `TaskEventPublisher` directly or retain dual paths; workflow-step mutations use their publisher.
