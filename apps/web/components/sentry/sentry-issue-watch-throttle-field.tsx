...
export function MaxInflightTasksField({
  ...
  return (
    <div className="space-y-1.5">
      ...
      <p className="text-xs text-muted-foreground">{t("sentry:capOnOpenTasksCreatedBy")}</p>
      <Input
        type="number"
      ...
      />
      {invalid && (
        <p className="text-xs text-destructive">{t("sentry:enterAPositiveIntegerOrLeave")}</p>
      )}
    </div>
  );
}
